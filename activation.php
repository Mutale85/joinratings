...
if (isset($_GET['userid']) && isset($_GET['email']) && isset($_GET['pass'])) {
	...
	$query = $connect->prepare("SELECT * FROM members WHERE id = ? AND email = ?  AND password = ? ");
	$query->execute(array($userid, $email, $password));
	$count = $query->rowCount();
	if ($count > 0) {
		$update = $connect->prepare("UPDATE members SET activate = ? WHERE id = ? AND email = ? AND password = ? ");
